...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    CGFloat textLayerInterval = (self.frame.size.width-20) / ( _numOfMenu * 2);
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //indicator
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(self.frame.size.width-10, self.frame.size.height/2)];
...
- (instancetype)initWithOrigin:(CGPoint)origin andSize:(CGSize)c_Size {
    self = [self initWithFrame:CGRectMake(origin.x, origin.y, c_Size.width, c_Size.height)];
    if (self) {
        _origin = origin;
        ...
        _tableView.rowHeight = c_Size.height+2;
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, origin.y, c_Size.width, c_Size.height)];
...
//        add separator shadow
        UIView *bottomShadow = [[UIView alloc] initWithFrame:CGRectMake(c_Size.width-20-0.5, 0, 0.5, c_Size.height)];
...
- (void)confiMenuWithSelectRow:(NSInteger)row {
    ...
    indicator.position = CGPointMake(self.frame.size.width-10, indicator.position.y);
}
